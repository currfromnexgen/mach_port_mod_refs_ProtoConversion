...
unsigned int EncryptFunction(unsigned int instr, unsigned int pc, unsigned int init, unsigned int constant, unsigned int mask) {
	...
	SET_OPCODE(result, GET_OPCODE(instr));
...
unsigned int JumpEncryption(unsigned int instr, unsigned int pc) {
	for (int i = 0; i <= MAXARG_A; i++) {
		int tempInstr = instr;
		SETARG_A(tempInstr, i);

		int guess = EncryptFunction(tempInstr, pc, _rotl(1, 13) | _rotr(1, 7), -10065, MASK1(SIZE_Bx, 0));
...
unsigned int setupval_encryption(unsigned int instr, unsigned int pc) {
	for (int c = 0; c <= MAXARG_C; c++) {
		...
		SETARG_C(temp_instr, c);

		int guess = EncryptFunction(temp_instr, pc, _rotl(1, 13) | _rotr(1, 7), 0x961C86, 0x3FC01FF);
